...
if mode == "AWS":
    ...
    local_parquet = os.path.join(BASE_DIR, "tmp/yellow.parquet")

    s3.download_file(bucket, raw_key, local_parquet)
    raw_parquet = local_parquet

    silver_out = f"{bucket}/dev/projects/nyc_taxi_etl/silver/trips_silver.parquet"
